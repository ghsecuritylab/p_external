...
public class HttpHandler extends URLStreamHandler {
    ...
    /// M: HttpCache method
    private static java.lang.reflect.Method sHttpCacheMethod;

    @Override protected URLConnection openConnection(URL url) throws IOException {

        /// M: Enable Http cache for specific URL @{
        try {
            if (sHttpCacheMethod == null) {
                PathClassLoader pcLoader = new PathClassLoader(
                        "/system/framework/mediatek-framework-net.jar",
                        ClassLoader.getSystemClassLoader());
                Class loadedClass = pcLoader.loadClass(
                        "com.mediatek.net.http.HttpCacheExt");
                sHttpCacheMethod = loadedClass.getDeclaredMethod(
                        "checkUrl", URL.class);
